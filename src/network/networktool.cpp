...
void NetworkTool::checkNewVersionInGitHubPayload(QJsonDocument& jsonDocument)
{
    ...
    static QRegularExpression releaseTagRegex;
    // Check if version start with 't' from test version
    // For everything we can presume normal release
    if(projectTag.contains('t') == 0) {
        releaseTagRegex.setPattern(QStringLiteral("(t)\\d+\\.\\d+"));
        qCDebug(NETWORKTOOL) << "Running test version:" << projectTag;
    } else {
        releaseTagRegex.setPattern(QStringLiteral("(v)\\d+\\.\\d+"));
        qCDebug(NETWORKTOOL) << "Running release version:" << projectTag;
    }

    // Check if this actual version is a real release
